...
num_col_fake = nrow(fake)
colnames(fake) <- c("splitted", "numFake")
num_col_true = nrow(true)
colnames(true) <- c("splitted", "numCred")
fake_sum <- sum(fake$numFake)
true_sum <- sum(true$numCred)
fake$probFake <- (fake$numFake+1)/(num_col_fake+fake_sum)
true$probCred <- (true$numCred+1)/(num_col_true+true_sum)
fake
true
...
total_fake_prob = sum(new_data$numFake)/sum(new_data$numCred)
total_cred_prob = 1 - total_fake_prob
                      
fake_stabilizer = sum(all_data$numFake) + nrow(new_data)
cred_stabilizer = sum(all_data$numCred) + nrow(new_data)
                      
credToFakeRatio = sum(true$numCred)/sum(fake$numFake)

                      
for (word in words){
    if (isTRUE(length(word))) {                    
...
```
```{r}
body_text <- subset(test, select=c("Body"))

sum(test$Label=="credible")
...
#print("Result precision:" + preciosion)
#print("Result recall:" + recall)
#print("Result f1-score:" + f1score)
