...
new_data <- merge(x = fake, y = true, by="splitted", all = TRUE)
fake %>% full_join(true, by"splitted")
new_data
       ...
       methods = list(
                    ...
                    fit = function(data)
                    {
                      ...
                      true <- tidy_text_true %>% count(splitted,sort=TRUE)
                         ...
                         #data_true <- data[data$Label == 'credible',]
                         #tidy_text_fake <- unnest_tokens(data_fake, 'splitted', 'Body', token="words")%>%
             #filter(!splitted %in% splitted_stop_words)
                         #tidy_text_true <- unnest_tokens(data_true, 'splitted', 'Body', token="words")%>%
             #filter(!splitted %in% splitted_stop_words)
                         #tidy_text_fake %>% count(splitted,sort=TRUE)
                         #tidy_text_true %>% count(splitted,sort=TRUE)
